(() => {
  ...
  var _Vector = class extends Events {
    ...
    rotate(radians) {
      const x = this.x;
      const y = this.y;
      const cos72 = Math.cos(radians);
      const sin72 = Math.sin(radians);
      this.x = x * cos72 - y * sin72;
      this.y = x * sin72 + y * cos72;
      return this;
    }
  };
  var Vector = _Vector;
  ...
  function renderArcEstimate(ctx, ox, oy, rx, ry, startAngle, endAngle, clockwise, xAxisRotation) {
    ...
    for (let i = 0; i < Constants.Resolution; i++) {
      ...
      if (xAxisRotation !== 0) {
        const cos72 = Math.cos(xAxisRotation);
        const sin72 = Math.sin(xAxisRotation);
        const tx = x - ox;
        const ty = y - oy;
        x = tx * cos72 - ty * sin72 + ox;
        y = tx * sin72 + ty * cos72 + oy;
      }
      ctx.lineTo(x, y);
    }
  }
  ...
  function dilute(component, amount) {
    return lerp2(component, 255, amount);
  }
  function lerp2(source, target, t) {
  ...
  var _Matrix2 = class extends Events2 {
    ...
    rotate(Number2) {
      const c = cos7(Number2);
      const s = sin7(Number2);
      return this.multiply(c, -s, 0, s, c, 0, 0, 0, 1);
    }
    translate(x, y) {
      return this.multiply(1, 0, x, 0, 1, y, 0, 0, 1);
  ...
  var Matrix4 = _Matrix2;
  __publicField(Matrix4, "Identity", [
    1,
    0,
    0,
    0,
  ...
  var _ZUI = class {
    constructor(group, domElement) {
      ...
      this.surfaceMatrix = new Matrix4();
      this.surfaces = [];
      this.reset();
      this.updateSurface();
